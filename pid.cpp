...
void straightPid(ie::Motion& motion, float target, int mile, int speed, float gain){
    ...
    pid(motion, target, mile, speed, 0.030*gain, 0.0001*gain, 0.019*gain*0.2, false); // 弱カーブP弱
}

void gentleCurvePid(ie::Motion& motion, float target, int mile, int speed, float gain){
    // (motion, 閾値, 距離, 速度,  kp, ki, kd,   isRight);
    pid(motion, target, mile, speed, 0.038*gain, 0.0001*gain, 0.017*gain*0.2, false); // 弱カーブP弱
}

void sharpCurvePid(ie::Motion& motion, float target, int mile, int speed, float gain){
    // (motion, 閾値, 距離, 速度,  kp, ki, kd,   isRight);
    pid(motion, target, mile, speed, 0.065 * gain, 0.0007 * gain, 0.030 * gain*0.2, false); // I弱D強
}
